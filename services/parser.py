...
def _detect_existing_loan(text: str) -> tuple[str | None, str]:
    negative_patterns = [
        r"(기존|현재|이전)?\s*(대출|정책\s*자금|정책자금|융자).{0,12}(없|미이용|안\s*받|받은\s*적\s*없|이용한\s*적\s*없)",
        r"(없|미이용|안\s*받|받은\s*적\s*없|이용한\s*적\s*없).{0,12}(대출|정책\s*자금|정책자금|융자)",
    ]
    for pattern in negative_patterns:
        match = re.search(pattern, text)
        if match:
            return "없음", f"원문에 '{match.group(0)}' 언급"
    ...
    for phrase in positive_phrases:
        if phrase in text:
            return "있음", f"원문에 '{phrase}' 언급"

    mention_patterns = [
        r"(기존|현재|이전|받은|받고|이용|사용).{0,12}(대출|정책\s*자금|정책자금|융자)",
        r"(대출|정책\s*자금|정책자금|융자).{0,12}(기존|현재|이전|받은|받고|이용|사용)",
    ]
    for pattern in mention_patterns:
        match = re.search(pattern, text)
        if match:
            return "있음", f"원문에 '{match.group(0)}' 언급"
...
def parse_consultation_text(raw_text: str) -> dict[str, Any]:
    ...
    loan, ev = _detect_existing_loan(text)
...
def _normalize_llm_result(data: dict[str, Any]) -> dict[str, Any]:
    ...
    for field in EXTRACTABLE_FIELDS:
        ...
        elif field == "existing_loan" and normalized.get(field) in ("있음", "없음"):
...
기존 대출, 현재 대출, 이전 대출, 정책자금 이용처럼 기존 금융지원 이용 이력이 언급되면 existing_loan 값을 '있음' 또는 '없음'으로 판단하고 field_status는 '확인됨'으로 둬라.
